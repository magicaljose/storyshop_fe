...
import {
  ...
  Checkbox, FormControl, NativeSelect, Fab, Radio, RadioGroup
  ...
  if (files.length === index) {
    ...
    currentSlide: slide,
  	...
  	handleAvatarChange = index => event => {
  		...
  		this.updateBuilder({cardAvatar: cardChanged});
  	...
  	render(){
    	...
    	const {
    		fields, tags, desc, charVal, relType, birthD, deathD, showSocialChar, defaultCrop,
			prevFields, progress, imgIndex, direction, rndm, rolling, showLite, cropStartOn, uploadingAvatar
		} = this.state;
